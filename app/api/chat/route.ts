...
export async function POST(req: Request) {
  ...
  const response = await openai.createChatCompletion({
    ...
    messages: [
      {
        role: 'user',
        content: `Generate a markdown document no more than 1500 characters, comprehensive and focused "chatgpt custom instructions" style prompt based on the "${instructions}" given and following ${
          type === 'user'
